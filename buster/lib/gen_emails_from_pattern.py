...
def gen_emails_from_pattern(input_info,email_pattern):
	...
	domain_list=[]		
	with open('/var/buster/email-providers.json','r') as json_file:
	...
	if(domain_list != [] and user_list != []):
		...
		print("we couldnt find the email provider in our database,if you know it pass it with the -p option and let us know so we can add it to the database later!")
