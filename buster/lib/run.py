...
def start():
	...
	if(args.email and args.first and args.middle and args.last and args.birthdate and('*' in args.email)):
		...
		hunter_rate_msg_sent=False
		if(generated_emails != []):
			print("[=]Validating " + str(len(generated_emails)) + " possible emails")
			for email in generated_emails:
				email_info=validate_email(email,api_key)
				if(email_info["hunter_limit_reached"]==True and hunter_rate_msg_sent==False ):
					print("[=]Warning:You have reached your Hunter.io usage limit,the email validation process will continue but you wont get additional information on company emails")
					print("[=]Use an API key if you want those information")
					hunter_rate_msg_sent=True
		...
		email_info=validate_email(args.email,api_key)
		hunter_rate_msg_sent=False
		if(email_info["hunter_limit_reached"]==True and hunter_rate_msg_sent==False ):
			print("[=]Warning:You have reached your Hunter.io usage limit,the email validation process will continue but you wont get additional information on company emails")
			print("[=]Use an API key if you want those information")
		...
		if(email_info["hunter_limit_reached"]==True):
		...
		domain_list=[]		
		with open(os.path.abspath('/var/buster/email-providers.json'),'r') as json_file:
